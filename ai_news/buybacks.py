...
ai_reads_search.read_search(google_key, polygon_key, my_cse_id, query='stock', \
  orTerms='buyback repurchase', prompt_template=prompt_template, model=model)


prompt_template="The time now is "
prompt_template+=datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y")

prompt_template+=""". Based on the following search result, please answer yes or no about whether this constitues a NEW announcement of improved guidance, The announcement should say that guidance for an important metric is now higher than previously expected. If the announcement is not within 23 hours of the current time or simply an update about progress on existing guidance, respond no. Please also answer the full name of the company doing the announcement, the ticker and the time it was published. 
