...
fundamentals = fread("~/stonksanalysis/other_datasources/nasdaq_screener_foreign.csv") #from https://www.nasdaq.com/market-activity/stocks/screener
prices = data.table(tq_get(fundamentals$Symbol))
prices = prices[symbol %in% prices[,.N,symbol][N>35, symbol]]
...
prices[,volume_avg:= SMA(shift(volume,1,type='lag'), n = 30,) ]

prices[,lagging_corr:=
         runCor( us_delta, abroad_delta, 28),
       symbol]
prices[,lagging_corr2:=
         rollapplyr( .SD, 28, function(x) cor(x[,1], x[,2], method = 'spearman'), by.column=F, fill = NA  ),
       symbol, .SDcols=c('us_delta','abroad_delta')]

prices_metadata = merge(prices,fundamentals,by.x = 'symbol', by.y = 'Symbol',all.x = T)

prices_metadata[year(date)==2019 ,.(mean(lagging_corr,na.rm=T),.N),.(round(log(Volume)))][order(round)]
prices_metadata[day_premarket<.96 & lagging_corr2< -.4, .(mean(day_delta,na.rm=T),.N), year(date)][order(year)]
...
subsample = prices_metadata[log(volume+1) %between% c(7,11) 
